...
if ($env:BUILDTARGETS.Contains("#$buildEnv#")) {
  ...
  $sw = [Diagnostics.Stopwatch]::StartNew()
    Set-Location -Path .\Binner.PrintSpoolService\BinnerPrintSpoolServiceInstaller
    (Get-Content .\BinnerPrintSpoolServiceInstaller.iss).replace('MyAppVersion "0.0"', 'MyAppVersion "' + (($env:APPVEYOR_BUILD_VERSION).split('-')[0]) + '"') | Set-Content .\BinnerPrintSpoolServiceInstaller.iss
    (Get-Content .\BinnerPrintSpoolServiceInstaller.iss).replace('@NETFRAMEWORK@', "$framework") | Set-Content .\BinnerPrintSpoolServiceInstaller.iss
    Write-Host "Building installer using the following script:" -ForegroundColor cyan
    cat .\BinnerPrintSpoolServiceInstaller.iss
    .\build-installer.cmd
    if ($LastExitCode -ne 0) { Write-Host "Exiting - error code '$LastExitCode'"; exit $LastExitCode }
    Set-Location -Path ..\..\
  $sw.Stop()
...
$sw = [Diagnostics.Stopwatch]::StartNew()
  ...
  if ($env:BUILDTARGETS.Contains("#win-x64#")) {
    Get-ChildItem .\Binner.PrintSpoolService\BinnerPrintSpoolServiceInstaller\*.exe | % { Push-AppveyorArtifact $_.FullName }
  }
